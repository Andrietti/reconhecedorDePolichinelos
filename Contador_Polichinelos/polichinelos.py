...
video = cv2.VideoCapture('agachamento curto.mp4') # Variável que armazena a função de captura de video,
...
def polichinelos():
    ...
    while True: # Loop para rodar o video
        ...
        if points: # condição que vai verificar se a variavel points não está vazia
            ...
            omDX = int(points.landmark[pose.PoseLandmark.LEFT_SHOULDER].x*w)
            coDY = int(points.landmark[pose.PoseLandmark.RIGHT_ELBOW].y*h)
            coDX = int(points.landmark[pose.PoseLandmark.RIGHT_ELBOW].x*w)
            coEY = int(points.landmark[pose.PoseLandmark.LEFT_ELBOW].y*h)
            coEX = int(points.landmark[pose.PoseLandmark.LEFT_ELBOW].x*w)
